...
def _code_notes():
    """Build the code-translation section from the DB's status_dict + branch/role refs."""
    lines = ["## การแปลรหัสในระบบ (API คืนเป็นเลข/โค้ด ต้องแปลก่อนสื่อสารกับคน)"]
    for field, label in [("order_status", "สถานะออเดอร์"),
                         ("status_payment", "สถานะการชำระเงิน"),
                         ("status_send", "สถานะการส่ง")]:
        vals = con.execute("SELECT code, meaning_th FROM status_dict WHERE field=? ORDER BY code",
                           [field]).fetchall()
        if vals:
            lines.append(f"- {label}: " + " · ".join(f"{c}={m}" for c, m in vals))
    cus = con.execute("SELECT code, meaning_th FROM status_dict WHERE field='cus_type'").fetchall()
    if cus:
        lines.append("- ประเภทลูกค้า: " + " · ".join(f"{c}={m}" for c, m in cus))
    docs = con.execute("SELECT code, meaning_th FROM status_dict WHERE field='doc_code'").fetchall()
    if docs:
        lines.append("- รหัสเอกสาร: " + " · ".join(f"{c}={m.split('(')[0].strip()}" for c, m in docs))
    branches = con.execute("SELECT code, name_th FROM ref_value WHERE ref_table='Factory'").fetchall()
    if branches:
        lines.append("- สาขา/โรงงาน: " + " · ".join(f"{c}={n}" for c, n in branches))
    lines.append("- อย่าคาดเดาความหมายรหัสเอง ถ้าไม่แน่ใจให้ยึดข้อมูลในระบบ/ถามหัวหน้างาน")
    return "\n".join(lines) + "\n"


CODE_NOTES = None  # built lazily in main() after con is ready
...
def main():
    global CODE_NOTES
    CODE_NOTES = _code_notes()   # build from DB now that con is ready

// Sales-assistant QA battery — drives real quote conversations through the WebUI.
// Reads cases from sales_cases.json: each case = new chat with pingzy-sales; if an
// ask-set stepper appears it answers every question (case.answers._freetext for typed
// questions, otherwise the FIRST option), goes through review, submits once, then
// checks must[]/mustNot[] regexes against the final assistant text. Records whether
// the model asked first, re-asked after answering, leaked markers, or returned empty.
// Runs cases SEQUENTIALLY (the brain has a shared 4-slot queue). Empty response
// (known 0-token transient) retries the case once in a fresh chat.
// Run: node sales_battery.js <email> <pw> [caseId ...]
const { chromium } = require('playwright');
const fs = require('fs');
const BASE = 'http://127.0.0.1:3000';
const [email, password, ...onlyIds] = process.argv.slice(2);
const DIR = process.env.OUTDIR || __dirname + '/runs';
const CASES = JSON.parse(fs.readFileSync(__dirname + '/sales_cases.json', 'utf8'))
...
async function runCase(browser, kase, attempt) {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/battery_${kase.id}.png`, fullPage: true });
    r.textTail = tail.slice(-900);
  } catch (e) {
    r.pass = false; r.failures.push('ERROR: ' + e.message);
    try { await page.screenshot({ path: `${DIR}/battery_${kase.id}_err.png`, fullPage: true }); } catch {}
  } finally {
    await page.close();
  }
  return r;
...
(async () => {
  ...
  await browser.close();
  fs.writeFileSync(`${DIR}/battery_results.json`, JSON.stringify(results, null, 2));
  const passed = results.filter(r => r.pass).length;
  console.log(`\n===== BATTERY: ${passed}/${results.length} PASS =====`);
